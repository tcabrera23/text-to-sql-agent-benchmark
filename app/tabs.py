"""Renderizado de las 5 pestañas de la app. Cada función recibe explícitamente
lo que necesita (cliente LLM, modelo, session_id) en vez de depender de
variables globales, para poder importarse y probarse de forma aislada."""
...
from benchmark.catalog import ARENA_TESTS, get_tests_by_level, TEST_STATS
from app.pricing import PRICING_TABLE, COST_ANALYSIS, RECOMMENDATIONS
from app.agents import run_chat_agent, run_dashboard_agent, run_arena_test


def render_chat_tab(llm_client, model_name: str, session_id: str):
    ...
    with col_chat:
        ...
        if prompt := st.chat_input("Ej: ¿Cuáles son los 5 artistas más vendidos?"):
            run_chat_agent(prompt, llm_client, model_name, session_id)
            ...
            for i, result in enumerate(st.session_state.query_results):
                ...
                if result["error"]:
                    ...
                    if st.button(f"Visualizar Consulta {i+1} en Dashboard", key=f"viz_{i}"):
                        ...
                        run_dashboard_agent(prompt, llm_client, model_name, session_id)
                        st.toast("Petición enviada al agente de dashboard. ¡Revisa la otra pestaña!")


def render_dashboard_tab(llm_client, model_name: str, session_id: str):
    ...
    if st.sidebar.button("Ejecutar Petición de Dashboard"):
        if dashboard_prompt:
            run_dashboard_agent(dashboard_prompt, llm_client, model_name, session_id)
        else:
...
def render_arena_tab(effective_openrouter_key: str, session_id: str):
    """Pestaña 3: Arena LLM."""
...
def render_arena_results_tab():
    """Pestaña 4: Resultados del Arena (lee data/arena_results.json)."""
...
def render_metrics_tab():
    """Pestaña 5: Métricas de Uso (lee data/metrics.csv, protegida por PIN)."""
